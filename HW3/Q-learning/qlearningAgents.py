...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        if (util.flipCoin(self.epsilon)): # if random number is less than epsilon, 
            action = random.choice(legalActions) # choose random action
        else:
            action = self.computeActionFromQValues(state) # choose best action
        return action
        # End your code
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        # Begin your code
        tdTarget = reward + self.discount * self.computeValueFromQValues(nextState)
        self.qTable[(state, action)] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * tdTarget
        # End your code
